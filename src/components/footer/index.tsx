...
export default function Footer({ className }: { className: string }) {
  ...
  return (
    <footer
      className={cn(
        'bg-background dark:border-border/14 border-border/7 w-full border-t px-4 pt-12',
        className,
      )}
      ref={ref}
    >
      <div className="mx-auto w-full max-w-(--breakpoint-lg)">
        ...
        <nav className="flex gap-24">
          <ul className="flex flex-col gap-1 pl-11">
            {SITE_CONFIG.pages.map(page => (
              <li
                key={page.link}
                className="dark:border-border/14 border-border/7 relative w-full border-b"
              >
                <Link
                  href={page.link}
                  className={cn(
                    'block w-full pb-1 pl-1 font-normal',
                    'before:bg-accent-foreground before:absolute before:bottom-0 before:left-0 before:h-[1px]',
        ...
        <p className="pt-8 pb-2 text-end">
          <small>
